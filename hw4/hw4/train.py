...
def compute_group_advantages(rewards: torch.Tensor, group_size: int, eps: float = 1e-6) -> torch.Tensor:
    # TODO(?V)(student): implement group-relative advantage normalization.
    ...
    extras = None
    excess = rewards.numel() % group_size
    if(excess) != 0:
        extras = rewards[-excess :]
        rewards = rewards[: -excess]
    ...
    for g in range(n_groups):
        ...
        rewards[g] = (r_g - mean_g) / (std_g + eps)
    ...
    if extras is not None:
        r_g1 = extras
        mean_g1 = r_g1.mean()
        std_g1 = r_g1.std(unbiased=False)
        if(std_g1 < eps):
            extra_a = torch.zeros_like(r_g1)
        else:
            extra_a = ((r_g1 - mean_g1) / (std_g1 + eps))
        rewards = torch.cat([rewards.view(-1), extra_a])
...
def maybe_normalize_advantages(advantages: torch.Tensor, enabled: bool, eps: float = 1e-6) -> torch.Tensor:
    # TODO(V)(student): if enabled, z-score normalize the full advantage vector:
    ...
    mean_a = advantages.mean()
    std_a = advantages.std(unbiased=False)
    if std_a < eps:
        return torch.zeros_like(advantages)
    return (advantages - mean_a) / (std_a + eps)
