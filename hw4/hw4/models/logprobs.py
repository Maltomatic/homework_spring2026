...
def compute_per_token_logprobs(
    ...
    *,
    ...
    # TODO(V)(student): implement next-token log-probs aligned to target tokens.
    # Notation:
    ...
    # autograd graph.
    offset_logits = logits[:, :-1, :].contiguous()
    targets = input_ids[:, 1:].contiguous()
    if not enable_grad:
        offset_logits = offset_logits.detach()
        targets = targets.detach()
    per_token_nll = F.cross_entropy(offset_logits.view(-1, V), targets.view(-1), reduction="none")
    per_token_logprobs = -per_token_nll.view(B, L - 1)
...
def build_completion_mask(
    ...
    # TODO(V)(student): return a float mask of shape [B, L-1] on the same device as
...
def approx_kl_from_logprobs(
    ...
    # TODO(V)(student): implement a masked mean KL proxy. All three inputs have shape
