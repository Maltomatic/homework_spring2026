...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        # TODO(V)(Section 2.4): get the action from the critic using an epsilon-greedy strategy
    ...
    def update_critic(
        ...
        with torch.no_grad():
            # TODO(V)(Section 2.4): compute target values
            next_qa_values = self.target_critic(next_obs)

            if self.use_double_q:
                # TODO(V)(Section 2.5): implement double-Q target action selection
            ...
            target_values = reward + self.discount * next_q_values * (torch.ones_like(done) ^ done)
            assert target_values.shape == (batch_size,), target_values.shape
            # ENDTODO

        # TODO(V)(Section 2.4): train the critic with the target values
        qa_values = self.critic(obs).gather(1, action[:, None]).squeeze(1)
        q_values = qa_values.detach()
    ...
    def update(
        ...
        """
        # TODO(V)(Section 2.4): update the critic, and the target if needed
