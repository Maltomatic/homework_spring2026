...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            # TODO(V)(Section 3.5): Initialize log_alpha, alpha_optimizer, and target_entropy
            # Hint: Initialize log_alpha to log(temperature) so alpha starts at the given temperature
            self.log_alpha = nn.Parameter(torch.tensor(np.log(temperature), dtype=torch.float32, requires_grad=True, device=ptu.device))
            self.alpha_optimizer = torch.optim.AdamW([self.log_alpha], lr=alpha_learning_rate)
    ...
    def get_temperature(self) -> float:
        ...
        """
        if self.auto_tune_temperature:
            # TODO(V?)(Section 3.5): Return the current learned temperature
        ...
        with torch.no_grad():
            ...
            try:
                ...
                if torch.isnan(observation).any():
                    print("Observation contains NaN values")
                raise e
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        # TODO(V)(Section 3.6): Implement the "min" backup strategy (clipped double-Q).
    ...
    def update_critic(
        ...
        with torch.no_grad():
            # TODO(V)(Section 3.2): Sample from the actor and compute next Q-values
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                # TODO(V)(Section 3.3): Add entropy bonus to the target values for SAC
            ...
            # TODO(V)(Section 3.2): Compute the target Q-value
            target_values = self.discount * next_qs * (torch.ones_like(done) ^ done) + reward
            # ENDTODO
        ...
        # TODO(V)(Section 3.2): Update the critic
        # Predict Q-values
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # TODO(V?)(Section 3.3): Compute the entropy of the action distribution.
        # Note: Think about whether to use .rsample() or .sample() here...
        sampling = action_distribution.rsample()
        log_prob = action_distribution.log_prob(sampling)
        if(log_prob.ndim > 1):
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # TODO(V)(Section 3.4): Sample actions using reparameterization (replace the placeholder below)
        # Note: Think about whether to use .rsample() or .sample() here, and why...
        action = self.actor(obs).rsample()
        assert action.shape == (batch_size, self.action_dim), action.shape
        # ENDTODO

        # TODO(V)(Section 3.4): Compute Q-values for the sampled state-action pair (replace the placeholder below)
        q_values = self.critic(obs, action)
        assert q_values.shape == (self.num_critic_networks, batch_size), q_values.shape
        # ENDTODO
        ...
        # TODO(V)(Section 3.4): Compute the actor loss (replace the placeholder below)
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        # TODO(V)(Section 3.3): Add the entropy bonus to the actor loss: loss -= [your entropy bonus here]
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        # TODO(V)(Section 3.5): Implement dual gradient descent for temperature tuning
        alpha = self.log_alpha.exp()
        alpha_loss = -(alpha * (log_prob + self.target_entropy).detach()).mean()
        # if alpha_loss.isnan():
        #     print("\t\tNaN ALPHA LOSS")
    ...
    def update(
        ...
        critic_infos = []
        # TODO(V)(Section 3.2): Update the critic for num_critic_updates steps
        for _ in range(self.num_critic_updates):
            info = self.update_critic(observations, actions, rewards, next_observations, dones)
            critic_infos.append(info)
        # ENDTODO

        # TODO(V)(Section 3.3): Enable the actor update (once you have implemented entropy)
        actor_info = {}
        ...
        # TODO(V)(Section 3.2): Perform either hard or soft target updates.
        # Relevant variables:
        #  - step
