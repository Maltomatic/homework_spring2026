...
def sample_trajectory(
    ...
    while True:
        ...
        # TODO(V) use the most recent ob to decide what to do
        ac = policy.get_action(ob)

        # TODO(V): take that action and get reward and next ob
        # print(f"observation: {ob} \naction: {ac}")
        # print(f"observation shape: {ob.shape} \naction shape: {ac.shape}")
        next_ob, rew, done, info = env.step(ac[-1])

        # TODO(V) rollout can end due to done, or due to max_length
        steps += 1
